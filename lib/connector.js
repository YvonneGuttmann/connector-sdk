...
exports.Connector = class Connector {
    ...
    static async init(context){
        ...
        try{
            const fs = require("fs");
            if(context.testFlow) {
                BL.uiTemplates = fs.readFileSync(path.join(process.cwd(), 'utils', 'bl','resources', 'ui-templates.json'));
            } else {
                BL.uiTemplates = fs.readFileSync(path.join(process.cwd(), 'resources', 'ui-templates.json'));
            }

        } catch (ex) {
            context.logger.error(`Failed to read ui-templates.json file: ${ex.message}.\n${ex.stack}`);
            throw ex;
        }
...
//        this.logger.info (`checking validity of the BL connector {${Object.keys(BL.prototype)}}`);
        ...
        return new Promise((resolve, reject) => {
            var processChunk = async (approvalsChunk, hasMore) => {
                ...
                if(approvalsChunk.data.approvals === undefined)
                    return resolve(Result.InternalError({}, { entity: "task", "taskType": "sync" ,message: `Failed to perform sync - there is no approvals property in BL result!` }));
    ...
    async mapUserIds(data){
        ...
        return Result(await this.BL.mapUserIds(data, {}));
    }
