    ...
    async _performAction (type, data, options) {
        ...
        try {
            ...
            if (!BL.settings.selfValidation) {
                ...
                fetchedApproval = this.addApprovalData(rawApproval);
            ...
            try {
                ...
                err.error = ex.error || "CONNECTOR_ERROR";
                this.logger.info(`action failed. BL throw exception: ${err.error}`);
                if(ex.approval && err.error === "APPROVAL_DATA_MISMATCH") {
                    rawApproval = ex.approval;
                    fetchedApproval = this.addApprovalData(rawApproval);
                    err.fetchedApproval = fetchedApproval;
                } else if(err.error === "APPROVAL_NOT_EXIST") {
                    rawApproval = null;
                }
                err.details = ex;
                throw err;
            ...
            try {
                ...
                if (rawUpdatedApproval) {
                    updatedApproval = this.addApprovalData(rawUpdatedApproval);
...
//        this.logger.info (`checking validity of the BL connector {${Object.keys(BL.prototype)}}`);
    ...
    async sync(action){
        ...
        return new Promise((resolve, reject) => {
            ...
            var fetchRes = this.BL.fetch({ signatureList : sigList, fetchType : action.syncType, mappedUsers : action.mappedUsers }, (err, approvalsChunk, hasMore) => {
    ...
    async getApproval(data, options = {}){
        var rawUpdatedApproval = await this.BL.getApproval(data.approval, options),
