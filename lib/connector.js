...
exports.Connector = class Connector {
    ...
    async _performAction (type, data, options) {
        ...
        var fetchedApproval, updatedApproval, rawApproval, rawUpdatedApproval, uiTemplate;
        ...
        try {
            ...
            if (!this.BL.settings.selfValidation) {
                ...
                uiTemplate = await options.getUITemplate(fetchedApproval.schemaId);

                let dataValidationError = this._validateApprovalData(fetchedApproval, uiTemplate, logger);

                if(dataValidationError) {
                    var error = new Error (`Action failed! approval validation data was failed: ${dataValidationError}`);
                    logger.debug(`Before:\n${JSON.stringify(rawApproval)}.\nAfter:\n${JSON.stringify(fetchedApproval)}`);
