...
exports.Connector = class Connector {
    ...
    _createApprovalSignature(approval){
        var signatureTemplate = Configuration.getConfig().signatureTemplate;
    ...
    async sync (options){
        ...
        logger.info (`synching hash list (length = ${this.signatureList.length})`);
        var approvals;
        approvals = await this.BL.fetch({logger});
        approvals.forEach (this._addApprovalData.bind(this));
    ...
    async approve (data, options){
        ...
        try {
            ...
            this._addApprovalData(fetchedApproval);
            if (fetchedApproval.signature == data.approval.signature) {
                ...
                updatedApproval && this._addApprovalData(updatedApproval);
