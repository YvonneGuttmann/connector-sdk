...
var BL, config, transform, signatureList = [], cryptoData;
...
exports.Connector = class Connector {
    constructor(options) {
		...
		if (config.separateBlLog && process.env.logStream == "file")
			blLogger = this._blLogger = this.logger.seperateFile("bl.log", { consoleMode : "none" });

        this.BL = new BL({
			logger : blLogger,
			logPath : process.env.logStream === "file" && blLogger._fileName && path.dirname(blLogger._fileName),
			flow: testFlow
		});
    }
    ...
    static async init(context){
        ...
        cryptoData = context.config.caprizaConfig && context.config.caprizaConfig.crypto;
        ...
        try {
            ...
            context.logger.log(`transformer.js does not exist exists`);
        ...
        return new Promise((resolve, reject) => {
            var processChunk = async (approvalsChunk, hasMore) => {
                ...
                if(approvalsChunk.data.approvals === undefined)
                    return resolve(new Result.InternalError({}, { entity: "connector", message: `Failed to perform sync - there is no approvals property in BL result!` }));
            ...
            var fetchRes = this.BL.fetch({ signatureList : sigList, fetchType : action.syncType, state : action.state,  mappedUsers : action.mappedUsers }, (err, res, hasMore) => {
    ...
    async additionalActions (data) {
        if(!this.BL[data.action]) {
            ...
            return new Result.NotImplemented({}, { entity: 'task', message: `${data.action} was not implemented in the BL` });
        }
    ...
    async downloadAttachment (data){
        ...
        if(res instanceof Result.Success && res.data.data === undefined)
            return new Result.InternalError({}, { entity: "connector", message: `Failed to perform downloadAttachment - there is no data property in BL result!` });
        res.addMeta("entity", "attachment");
        return res;
    }

    async authenticate (data){
        if(data.credentials.appPublic) {
            data.credentials = decryptData(data.credentials);
        }

        try {
            var res = Result(await this.BL.authenticate(data.credentials, {}));
        } catch(ex) {
            return new Result.InternalError({}, { entity: "connector", message: `${ex.message || ex}` });
        }

        if(res instanceof Result.Success && res.data.authenticated === undefined)
            return new Result.InternalError({}, { entity: "connector", message: `Failed to perform authenticate - there is no authenticated property in BL result!` });
        res.addMeta("entity", "user");
		res.addMeta("authenticated", !!res.data.authenticated);
        return res;
    }
    ...
    async mapUserIds(data){
		...
		if(res instanceof Result.Success && !(res.data.users instanceof Array)) {
			this.logger.error('Failed to perform mapUserIds - there is no users property in BL result or users is not an array!');
			return new Result.InternalError({}, { entity: "connector", message: `Failed to perform mapUserIds - there is no users property in BL result or not an array!` });
		}
		return res;
    }
    ...
    _addApprovalData(approval) {
        ...
        try {
            var rawApproval = JSON.stringify(approval);
    ...
    async _performAction (type, data) {
        this.logger.info (`Performing action '${type}' on ${data.approval.id}`);
		...
		if (!currentApproval)
			return new Result.InternalError({}, { entity : "sdk", message : "Approval not found in signature list" });
        ...
        if(data.credentials && data.credentials.appPublic) {
            data.credentials = decryptData(data.credentials);
        }
        ...
        if(!(actionResult instanceof Result.Success)) {
            ...
            try {
                ...
                this.logger.error(`mini sync failed due to BL exception: ${(ex && ex.stack) || JSON.stringify(ex)}`);
                miniSyncResult = new Result.InternalError({}, { entity : "connector" });
            }
    ...
    async _getApproval(data, options = {}) {
        ...
        if(res instanceof Result.Success && res.data.approval === undefined)
            return new Result.InternalError({}, { entity: "connector", message: `Failed to perform getApproval - there is no approval property in BL result!` });
...
function decryptData(data) {
    if(cryptoData && cryptoData.publicKey && cryptoData.privateKey) {
        if(data.connPublic !== cryptoData.publicKey) {
            throw "Decryption error: connector pubic key is not equal to the connector pubic key that provided from the app";
        }
        const cipher = require('./encryption');
        let res = cipher.decryptData({ privateKey: cryptoData.privateKey, publicKey: cryptoData.publicKey }, data.appPublic, data.iv, data.data );
        return JSON.parse(res);
    } else {
        throw "Decryption error: trying to decrypt data but there is no cryptoData";
    }
}
