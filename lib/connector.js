...
exports.Connector = class Connector {
    ...
    _addApprovalData (approval) {
        if (this.config._controller.runtimeConfig.schemaTransformer) approval = jslt.transform(approval, this.config._controller.runtimeConfig.schemaTransformer);
    ...
    async init(context){
        this.config = context.Configuration.getConfig();
        this.config.runtimeConfig = await context.com.getRuntimeConfiguration({logger: context.logger});

        return this.BL.init(context);
    }
