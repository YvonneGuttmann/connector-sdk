...
exports.Connector = class Connector {
    ...
    async additionalActions (action, data, options) {
        if(!this.BL[action] || !this.BL.settings.additionalActions || !this.BL.settings.additionalActions.includes(action)) throw `${action} was not implemented in the BL`;
        return this._performAction(action, data, options);
    }
