...
exports.ComManager = class ComManager{
    constructor(connector, props){
        ...
        if (!this.config || !this.config.controllerConfig || !this.config.controllerConfig.apiUrl) throw `Missing Capriza configuration. \nCannot load connector. \nYou may want to load connector locally`;
    ...
    configureProxy() {
        if(this.config.controllerConfig.proxy) {
            require('proxying-agent').globalize(this.config.controllerConfig.proxy);
    ...
    async start(){
		...
		var runPartialSync = async () => {
			...
			try {
				await Tasks["sync"].call(this, {}, { logger, syncType : "partial" });
			} catch(ex) {
				logger.error(`Error running partialSync ${syncCounter}, ex: ${ex}`);
			} finally {
    ...
    async _performTask(task){
        ...
        try{
			...
			await Promise.race([ taskFunc.call(this, task.data, { logger: taskLogger }), this._taskTimeout(taskLogger) ]);
        }
        catch (err){
            ...
            if (err.approvalSyncResult) {
                ...
                await this._sendApprovals(err.approvalSyncResult, {logger: taskLogger});
            }
        }
...
var Tasks = Object.assign(Object.create(null), {
    async sync(data, options){
		...
		await this.connector.sync({
			...
			send : approvals => { return sendPromise = sendPromise.then(() => this._sendApprovals(approvals, options)); }
		});
		return sendPromise;
	},
    ...
    async approve(data, options) {
		await this._getBackendApproval({ logger: options.logger, id: data.approval.id });
		...
		await this._sendApprovals(approvalSyncResult, options);
	},
    ...
    async getAttachment(data, options) {
