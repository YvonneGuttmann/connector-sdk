...
axios.defaults.timeout = 15 * 60000;
...
exports.ComManager = class ComManager{
    ...
    async monitorMemoryUsage() {
        ...
        if (toMB(this.memoryUsage.rss) > this.config.memoryMaxLowerLimit) {
            ...
            this.logger.error (`Memory usage reached upper limit: ${toMB(this.memoryUsage.rss)} (Upper limit: ${this.config.memoryMaxUpperLimit})`);
    ...
    async pollForTask(){
		...
		if (this.shuttingDown) {
		    this.logger.info (`Shutting down process.. Current task count: ${activeTasksCount}. Not polling.`);
        ...
        try {
			var task = await axios.get(`${this.apiUrl}/connectors/tasks`, {
				timeout : 60000,
