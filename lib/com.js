...
exports.ComManager = class ComManager{
    constructor(connector, props){
        ...
        this.syncver = 0;
        this.apiUrl = Configuration.getConfig()._controller.apiUrl;
        this.data = [];
        this.dataCache = {};
    ...
    async register(){
        try{
            this.requestHeaders = {
                "x-capriza-api-key" : Configuration.getConfig()._controller.creds.apiKey,
                "x-capriza-secret" : Configuration.getConfig()._controller.creds.apiSecret
            };
        }
        catch (e){
    ...
    async getRuntimeConfiguration(props = {}){
        //todo: Load runtime config if not exist (Currently using Dror hack)
        var connectorName = process.cwd().split("/").split("\\").pop() + ".1.0.json";
        var obj = await axios.get(`http://schemas.capriza.com/runtime-configs/${connectorName}`, { headers : this.requestHeaders });

        props.logger.log (`Runtime configuration: ${obj.data}`);
