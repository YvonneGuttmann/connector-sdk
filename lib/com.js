...
exports.ComManager = class ComManager{
    ...
    async start(){
		...
		var runPartialSync = async () => {
			try {
				var task = new this.TaskClasses.InternalTask({ id : "partialSync-" + syncCounter, type : "sync", syncType : "partial" }, this);
				await task.execute();
			} catch(ex) {
				this.logger.error(`Error running partialSync ${syncCounter}, ex: ${ex}`);
			} finally {
