...
exports.ComManager = class ComManager{
    ...
    async start(){
        ...
        this.connector.init({ config: this.config, logger });
    }
    ...
    async getConfiguration(props = {}){
        ...
        try {
            res = await axios.get({
                method: "POST",
                url: `${this.apiUrl}/connector/current`,
        ...
        if (!res) return this.logger.info("No configuration was fetched from the backend");

        if (res.controllerConfig) this.config.controllerConfig = Object.assign (res.controllerConfig, this.config.controllerConfig.data);
        this.config.blConfig = res.blConfig.data;
    }
