...
var Logger = exports.Logger = class Logger {
    constructor({ consoleMode, fileMode, fileName, chindings, stream }) {
		...
		if (consoleMode == "json")
			this._console = pino(Object.assign({timestamp: pino.stdTimeFunctions.slowTime, level: 'debug'}, chindings), process.stdout);
		...
		if (fileMode) {
			if (stream) {
				...
				if (fileMode == "json")
					this._logger = pino(Object.assign({timestamp: pino.stdTimeFunctions.slowTime, level: 'debug'}, chindings), this._stream);
			}
		}
    }
	...
	debug(msg) {
		...
		if (this._consoleMode == "json")
			...
			this._console.info(textMsg = `${getTimestamp()} DEBUG  - ${msg}`);
