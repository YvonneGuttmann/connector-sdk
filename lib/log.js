...
var Logger = exports.Logger = class Logger {
    constructor({ consoleMode, fileMode, fileName, chindings, stream }) {
		...
		if (consoleMode == "json")
			this._console = pino({timestamp: pino.stdTimeFunctions.slowTime, level: 'debug'}, process.stdout).child(chindings);
		...
		if (fileMode) {
			if (stream) {
				...
				if (fileMode == "json")
					this._logger = pino({timestamp: pino.stdTimeFunctions.slowTime, level: 'debug'}, this._stream).child(chindings);
			}
		}
    }
