...
module.exports = class BackendAPI {
    ...
    async sendConnectorTaskTypes(taskTypes) {
        return this._request("patch", `/connectors/${this.config.connectorId}`, {taskTypes});
    }

	async sendFeedback(msg, feedbackUrl, zipFile) {
		...
		return axios.post(feedbackUrl, bodyFormData, { headers : bodyFormData.getHeaders() });
	}
