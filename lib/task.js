	...
	class Task {
		...
		async execute() {
			...
			try {
				...
				if (!taskFunc && taskType.indexOf('action') === -1) {
					var error =  new Error(`Unknown task type ${taskType}`);
					error.error = `TASK_INVALID`;
					throw error;				
				}

				if(taskType.indexOf('action') === 0) taskFunc = handlers['additionalActions'];
			...
			if (this._updatingProgressTimer) clearInterval(this._updatingProgressTimer);
			return this.backend.updateTask(this.id, {succeeded: !taskError, payload: (taskError && taskError.error) ? Errors.get(taskError.error) : taskError});
		}
		
		async abort() {
			this.logger.log(`Terminating task ${this.id} of type ${this.data.type}`);
			return this.backend.updateTask(this.id, { succeeded : false });	
		}
