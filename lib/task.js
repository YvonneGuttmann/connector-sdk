		...
		async execute() {
            ...
            if(!(taskResult instanceof Result.Success) && config.feedbackStatuses.includes(taskResult.status) && process.env.logStream === "file") {
                ...
                if(feedbackCounter <= config.feedbackMax){
                    this.logger.info(`Sending feedback. Task status: ${taskResult.status}`);
                    await  this._archiveLogs().then(zipPath => this.backend.sendFeedback(zipPath, {taskId: this.id, taskType: this.data.type, status: taskResult.status, auto: true}, this.logger));
