...
const defaultConfig = {
	...
	feedbackMax             : 2                    //max 10 feedback per hour
		...
		async execute() {
            ...
            if(!(taskResult.status instanceof Result.Success) && config.feedbackStatuses.includes(taskResult.status) && config.feedbackUrl && process.env.logStream === "file") {
                ...
                if(feedbackCounter <= config.feedbackMax){
                    ...
                    this.logger.log(`more then ${config.feedbackMax} feedbacks at the last ${config.feedbackInterval} ms. not sending feedback.`)
                }
            }
