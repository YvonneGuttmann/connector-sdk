...
module.exports = function({ handlers, config, backendFactory, Connector, logger }) {
	...
	class Task {
		constructor(data) {
			...
			this.logger = log.child({ taskType : data.type, taskId : data.id });			
		...
		async execute() {
			...
			try {
                ...
                if (!taskFunc && !taskType.startsWith('action$')) {
                    ...
                    taskResult = new Result.InternalError({}, { entity: "sdk", message: `Unknown task type` });
                } else {
				...
				taskResult = new Result.InternalError({}, { entity: "sdk", message: `Unexpected error: ${exceptionData}` });
			}

			taskResult.addMeta("taskType", taskType);
            ...
            var shouldSendFeedback = !(taskResult instanceof Result.Success) && config.feedbackStatuses.includes(taskResult.status) && process.env.logStream === "file";
            if (shouldSendFeedback) {
                feedbackCounter++;
                if(feedbackCounter <= config.feedbackMax) {
					taskResult.addMeta("feedbackTriggered", true);
                    this.logger.info(`Auto-feedback triggered. Task status: ${taskResult.status}`);
                } else {
                    shouldSendFeedback = false;
                    this.logger.info(`Auto-feedback threshold reached, not sending feedback`);
                }
            }
				
            await this._updateTask({ status : taskResult.status, meta : taskResult.meta });
            await this._destroy();

            if (shouldSendFeedback)
            	this._sendFeedback({taskId: this.id, taskType: this.data.type, status: taskResult.status, auto: true});
		}
		...
		async _sendFeedback(metaData) {
            try {
                let zipPath = await this._archiveLogs();
                await this.backend.sendFeedback(zipPath, metaData, logger);
                logger.info(`Successfully sent auto-feedback for task=${this.data.type}-${this.id}`);
            } catch(ex) {
                logger.error(`Failed to send auto-feedback for task=${this.data.type}-${this.id}: ${ex}`);
            }
		}
