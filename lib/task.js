...
module.exports = function({ handlers, config, api, connector, logger }) {
	...
	class Task {
		...
		async execute() {
			...
			try {
				...
				if (err.approvalSyncResult) {
					...
					await api.sendApprovals(err.approvalSyncResult, { logger : this.logger });
				}
			}
			...
			if (this._updatingProgressTimer) clearInterval(this._updatingProgressTimer);
			return api.updateTask(this.id, {succeeded: !taskError, payload: (taskError && taskError.error) ? Errors.get(taskError.error) : taskError});
		}
		
		async abort() {
			this.logger.log(`Terminating task ${this.id} of type ${this.data.type}`);
			return api.updateTask(this.id, { succeeded : false });	
		}
		...
		_updateTaskInProgress() {
			...
			this._updatingProgressTimer = setInterval(async () => {
				...
				try {
					return await api.updateTaskInProgress(this.id);
				} catch (ex) {
