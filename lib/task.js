...
module.exports = function({ handlers, config, backendFactory, Connector, logger }) {
	...
	class Task {
		...
		async execute() {
            ...
            if(!(taskResult instanceof Result.Success) && config.feedbackStatuses.includes(taskResult.status) && process.env.logStream === "file") {
                feedbackCounter++;
                if(feedbackCounter <= config.feedbackMax){
                    this.logger.info(`Sending feedback. Task result: ${JSON.stringify(taskResult)}`);
                    this._archiveLogs().then(zipPath => this.backend.sendFeedback(zipPath, {taskId: this.id, taskType: this.data.type, status: taskResult.status}));
                    this.logger.info(`successfully sent feedback.`);
                }else{
...
/*            if(taskType === 'authenticate') {
		...
		_archiveLogs() {
			...
			return new Promise(resolve => {
				...
				archive.pipe(output);
				console.log("this.logPath=",this.logPath)
				console.log("path.dirname(this.logPath)=",path.dirname(this.logPath))
				archive.directory(path.dirname(this.logPath), false);
				archive.finalize();
				console.log("finish _archiveLogs")
			});
		}
	};
