		...
		async execute() {
            ...
            if(shouldSendFeedback) {
                try {
                    ...
                    this.backend.sendFeedback(zipPath, {taskId: this.id, taskType: this.data.type, status: taskResult.status, auto: true}, logger);
                    logger.info(`Successfully sent auto-feedback for task=${this.data.type}-${this.id}`);
                } catch(ex) {
                    logger.error(`Failed to send auto-feedback for task=${this.data.type}-${this.id}`);
                }
            }
		}
