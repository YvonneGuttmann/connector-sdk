...
module.exports = Object.assign(Object.create(null), {
    async sync(){
		...
		var connectorState = this.data.state;
		if (connectorState){
		...
		var taskResult = await this.connector.sync({
            ...
            state       : connectorState && connectorState.state,
        ...
        if (connectorState && connectorState.state && connectorState.state.update){
            delete connectorState.state.update;
            await this.backend.setConnectorState({ logger : this.logger, connectorState });
        }
        ...
        try {
            ...
            return new Result.InternalError({}, { entity: "sdk", message: ex.message });
        }
    ...
    async authenticate() {
        if (!this.data.volatile || !this.data.volatile.system) {
            ...
            return new Result.InternalError({}, { entity: "sdk", message: `Failed to perform authenticate task - data.volatile doesn't exist!` });
        }
    ...
    async mapUserIds() {
        ...
        if (res instanceof Result.Success && res.data.users && res.data.users instanceof Array){
            if(res.data.users.length > 0) this.backend.sendUserIds(res.data.users);
        } else {
            this.logger.error(`mapUserIds: Invalid response from connector: ${JSON.stringify(res)}`);
        }
        return res;
    },

    //Local task (for AS Inspector)
    async getApproval() {
        var data = this.data;
        var options = { logger : this.logger };
        var res = await this.connector.getApproval(data, options);
        if(res instanceof Result.Success) {
            //As Inspector Naughty hack!
            res.meta = res.data;
        }
    ...
    async sendFeedback() {
        ...
        if (await existDir("log")) {
            ...
            if (logFiles.length > 0) {
                ...
                await this.backend.uploadFeedback(data.uploadUrl, zipPath);
                ...
                this.logger.error(`logDirFiles = ${logDirFiles.log}`);
                this.logger.error(`logDirArchiveFiles = ${logDirFiles.log_archive}`);
                return new Result.NotFound({}, {message: msg});
            }
        } else {
            var msg = `log dir doesn't exist.`;
            this.logger.error(msg);
            return new Result.InternalError({}, { entity : "sdk", message: msg });
        }

        return new Result.Success();

        async function searchLog(taskId, dirName){
            var logFiles = logDirFiles[dirName] = await readDir(dirName);
    ...
    switch (action) {
        ...
        case "additionalActions":
            if (data.volatile){
                ...
                return new Result.InternalError({}, { entity: "sdk", message: "Trying to perform additionalActions but data.volatile was not provided" });
            }
            break;
        default:
            this.logger.error(`Trying to perform unknown action: ${action}`);
            return new Result.InternalError({}, {entity: "sdk", message: "Trying to perform unknown action" });
    }

    if(taskResult instanceof Result.Success || taskResult instanceof Result.DataMismatch || taskResult instanceof Result.NotFound) {
        if(taskResult.data.approvalsData && taskResult.data.approvalsData.approvals.length > 0) {
